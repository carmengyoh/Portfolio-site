...
- [ ] Role is explicit and unambiguous. On team projects, state exactly what *she* owned.
...
### 5. Scannability
- [ ] Works with zero hover; all key info is readable on touch devices.
...
- `case-study.css`: all `.cs-*` components. `styles.css` contains global tokens, nav, footer and cards.
- `images/flex/`: Flex assets including compressed demo video and poster.
...
- **CSS source order:** with equal-specificity rules, later wins. A modifier placed *above* the base rule silently loses. Bump specificity (`.cs-article.cs-article--tldr h2`) rather than relying on order.
...
- **Headless Chrome floors narrow widths at ~500px.** Requesting `--window-size=390` still lays out at 500 and crops the screenshot, so text looks clipped when it isn't. Don't report layout bugs from narrow headless shots without measuring `getBoundingClientRect` first.
...
- `.cs-progress` (side nav dots) is hidden below 1140px viewport by design, not because of a bug.
...
Screenshot via headless Chrome against `localhost` (not `file://`, because MIME types matter for video/iframes):
```
