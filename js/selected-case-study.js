(function () {
    ...
    var projects = {
        ...
        'document-upload': {
            ...
            outcome: 'The feature shipped in Q4 2025 and gave partner agents a structured way to provide evidence on a merchant’s behalf. It reduced the need to contact merchants again for documents their agent already held, while giving Operations clearer files tied to the requirement they needed to review.',
    ...
    function uploadDetailedHTML() {
        return section('Overview', project.subtitle + ' ' + project.problem) +
            ...
            '<section class="cs-article-section"><h3 class="cs-step-title">Step #2: Connecting the document manager to each application task</h3>' +
            ...
            '<section class="cs-article-section"><h3 class="cs-step-title">Step #4: Making progress, failure and completion visible</h3>' +
            ...
            '</div></section>' +
            '<section class="cs-article-section"><h3 class="cs-step-title">Step #5: Working with engineers to ship the workflow</h3>' +
    ...
    function iaDetailedHTML() {
        return section('Overview', project.subtitle) +
            ...
            '</div></section>' +
            decisionsHTML() + section('The solution', project.solution) + section('Outcome', project.outcome) + section('What I learned', project.reflection);
    }

    function detailedHTML() {
        if (project.visual === 'upload') return uploadDetailedHTML();
        if (project.visual === 'ia') return iaDetailedHTML();
        return section('Overview', project.subtitle) + section('Understanding the <em>problem</em>', project.problem) + section('How I approached it', project.process) + visualsHTML() + decisionsHTML() + section('The solution', project.solution) + section('Outcome', project.outcome) + deliveryHTML() + section('What I learned', project.reflection);
    }
